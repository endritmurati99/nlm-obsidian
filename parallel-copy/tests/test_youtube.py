...
from src.youtube import _build_source_arg, _clean_srt, _parse_metadata_line, VideoMeta
...
def test_build_source_arg_for_search_query():
    source_arg, mode = _build_source_arg("Claude Code MCP", 5)
    assert source_arg == "ytsearch5:Claude Code MCP"
    assert mode == "search"


def test_build_source_arg_for_direct_url():
    source_arg, mode = _build_source_arg("https://www.youtube.com/watch?v=abc123", 5)
    assert source_arg == "https://www.youtube.com/watch?v=abc123"
    assert mode == "url"
